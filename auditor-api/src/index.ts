...
// JSON patch not required for initial demo; SSE delta streaming can be added later
...
const sessionState: Map<string, AnalysisState> = new Map();

server.get('/health', async () => ({ ok: true }));

server.post('/session', async (req, reply) => {
  const sessionId = uuidv4();
  const state: AnalysisState = { sessionId, status: 'idle' };
...
function inferPurpose(text: string): string {
  if (/financial statements?/i.test(text)) return 'Financial Statement';
  if (/audit report/i.test(text)) return 'Audit Report';
  if (/tax/i.test(text)) return 'Tax Filing';
...
function inferHighlights(text: string): any {
  return { notes: text.length > 100 ? 'Large document' : 'Small document' };
}

function computeDummyMetrics(_text: string) {
  return {
